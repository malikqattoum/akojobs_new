				...
				<div class="col-md-9 page-content">
					<div class="inner-box">
						@if ($pagePath=='my-posts')
							...
							@if (!empty($userPackages) && $userPackages->count() != 0)
								<h4>{{t('my job post packages')}}</h4>
								<table class="table table-hover">
									<thead>
									  <tr>
										<th scope="col">{{t('package name')}}</th>
										<th scope="col">{{t('used posts')}}</th>
										<th scope="col">{{t('remaining posts')}}</th>
									  </tr>
									</thead>
									<tbody>
										@foreach ($userPackages as $package)		
											<tr>
											<td>{{$package->postPackage->name}}</td>
											<td>{{(int)$package->postPackage->post_num - (int)$package->valid_jobs_num}}</td>
											<td>{{$package->valid_jobs_num}}</td>
											</tr>
										@endforeach
									</tbody>
								  </table>
								...
								<table id="addManageTable" class="table table-striped table-bordered add-manage-table table demo"
									...
									<tr>
										...
										<td style="width:14%" class="add-img-td">
											<a href="{{ $postUrl }}">
												<img class="img-thumbnail img-fluid" src="{{ resize(\App\Models\Post::getLogo($post->logo), 'medium') }}" alt="img">
											</a>
										</td>
										<td style="width:58%" class="ads-details-td">
											<div>
												<p>
                                                    <strong>
                                                        <a href="{{ lurl('account/conversations/'.$post->id.'/applicants') }}" title="{{ $post->title }}">{{ \Illuminate\Support\Str::limit($post->title, 40) }}</a>
                                                    </strong>
											...
											<div>
												<strong>
													@if ($post->salary_min > 0 && $post->salary_max > 0)
														{!! $post->salary_min.' '.$post->salary_currency.' - '.$post->salary_max.' '.$post->salary_currency !!}
													@elseif ($post->salary_min > 0 && empty($post->salary_max))
														{!! $post->salary_min.' '.$post->salary_currency !!}
														{{-- {!! \App\Helpers\Number::money(' --') !!} --}}
													@elseif ($post->salary_max > 0 && empty($post->salary_min))
										...
										<td style="width:10%" class="action-td">
											<div>
												@if (in_array($pagePath, ['my-posts']) and $post->user_id==$user->id and $post->archived==0)
													<p>
														<a class="btn btn-default btn-sm" href="{{ $postUrl }}">
															<i class="icon-eye"></i> {{ t('view job post') }}
														</a>
													</p>
												@endif
												@if ($post->user_id==$user->id)
													<p>
                                                        <a class="btn btn-primary btn-sm" href="{{ editPostURL($post->id) }}">
                                                            <i class="fa fa-edit"></i> {{ t('Edit job post') }}
                                                        </a>
                                                    </p>
												@endif
												@if (in_array($pagePath, ['my-posts']) and isVerifiedPost($post) and $post->archived==0)
													<p>
														<a class="btn btn-warning btn-sm confirm-action" href="{{ lurl('account/'.$pagePath.'/'.$post->id.'/offline') }}">
															<i class="icon-eye-off"></i> {{ t('close job') }}
														</a>
													</p>
												@endif
												...
												<p>
                                                    <a class="btn btn-primary btn-sm" href="{{ lurl('account/conversations/'.$post->id.'/applicants') }}">
                                                        <i class="fa fa-users"></i> {{t('Applicants')}}
                                                    </a>
                                                </p>
											</div>
										</td>
