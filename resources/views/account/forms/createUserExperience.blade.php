        ...
        <div class="row">
            ...
            <div class="col-md-9 page-content">
                ...
                <div class="card uper">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('account.storeUserExperience') }}"
                            enctype="multipart/form-data">
                            <div class="form-group required" >
                                @method('POST')
                                @csrf
                                <label for="exp_title">{{t('Title')}} <sup>*</sup></label>
                                <input type="text" value="{{ old('exp_title') }}" name="exp_title" class="form-control">
                            </div>
                            <div class="form-group required">
                            ...
                            <div class="form-group required">
                                ...
                                <select class="form-control" name="exp_country" id="exp_country" placeholder="{{ t('What is your experience location?') }}"
                                    ...
                                    @foreach ($countriesKeys as $item)
                                        <option value="{{$item}}" {{ (old("exp_country") == $item ? "selected":"") }} >{{t($item,[],'residenceCountry')}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type='checkbox' onchange="checkToggler()" id="toggler_input" class="mt-1" value="1" {{ (old('present'))?'checked':'' }} name="present" />
                            </div>
                            <div class="form-group required">
                                <label for="exp_tasks">{{t('Tasks & responsibilities')}} <sup>*</sup></label>
                                <textarea name="exp_tasks" class="form-control">{{ old('exp_tasks') }}</textarea>
                            </div>
