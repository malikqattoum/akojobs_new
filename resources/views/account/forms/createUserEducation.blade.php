        ...
        <div class="row">
            ...
            <div class="col-md-9 page-content">
                ...
                <div class="card uper">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('account.storeUserEducation') }}"
                            ...
                            <div class="form-group required">
                                ...
                                <select class="form-control" name="edu_degree" id="edu_degree" placeholder="{{ t('Select') }}"
                                      value="{{ old('edu_degree') }}">
                                    <option value="" selected="">{{ t('Select') }}</option>
                                    @foreach ($degreesKeys as $item)
                                        <option value="{{$item}}" {{ (old("edu_degree") == $item ? "selected":"") }} >{{t($item,[],'degreesList')}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group required">
                                <label for="edu_institution">{{t('Organization/Institutes')}} <sup>*</sup></label>
                                <input type='text' placeholder="Ex. Harvard University" value="{{ old('edu_institution') }}" name="edu_institution" class="form-control" />
                            </div>
                            <div class="form-group" id="toggle_el">
                                <label for="edu_grad_date">{{t('Graduation date')}}</label>
                                <input type='date' value="{{ old('edu_grad_date') }}" name="edu_grad_date" class="form-control" />
                            </div>
                            <div class="form-group">
                                <label for="on_going">{{t('Still On Going')}}</label>
                                <input type='checkbox' onchange="checkToggler()" id="toggler_input" class="mt-1" value="1" {{ (old('on_going'))?'checked':'' }} name="on_going" />
                            </div>
