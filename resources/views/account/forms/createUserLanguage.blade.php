        ...
        <div class="row">
            ...
            <div class="col-md-9 page-content">
                ...
                <div class="card uper">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('account.storeUserLang') }}">
                            @method('POST')
                            @csrf
                            ...
                            <div class="form-group required">
                                ...
                                <select class="form-control" name="language" id="language" placeholder="{{ t('Select') }}"
                                    ...
                                    @foreach ($langKeys as $item)
                                        <option value="{{$item}}" {{ (old("language") == $item ? "selected":"") }} >{{t($item,[],'languages')}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group required">
                                <label for="lang_level">{{t('Level')}} <sup>*</sup></label>
                                <select class="form-control" name="lang_level" id="lang_level" placeholder="{{ t('Choose level') }}"
                                      value="{{ old('lang_level') }}">
                                    <option value="" selected="">{{ t('Select') }}</option>
                                    @foreach ($levelsKeys as $item)
                                        <option value="{{$item}}" {{ (old("lang_level") == $item ? "selected":"") }} >{{t($item,[],'langLevels')}}</option>
                                    @endforeach
                                </select>
                            </div>
