        ...
        <div class="row">
            ...
            <div class="col-md-9 page-content">
                ...
                <div class="card uper">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('account.storeUserTraining') }}">
                            @method('POST')
                            @csrf
                            <div class="form-group required">
                                <label for="training_name">{{t('Name')}} <sup>*</sup></label>
                                <input type="text" value="{{old('training_name')}}" name="training_name" class="form-control">
                            </div>
                            <div class="form-group required">
                                <label for="training_institution">{{t('Institution')}} <sup>*</sup></label>
                                <input type="text" value="{{old('training_institution')}}" name="training_institution" class="form-control">
                            </div>
                            <div class="form-group required">
                                <label for="training_completion">{{t('Date of completion')}}</label>
                                <input type="date" value="{{old('training_completion')?date('Y-m-d',strtotime(old('training_completion'))):''}}" name="training_completion" class="form-control">
                            </div>
