...
<div id="companyFields">
	...
	@if (isset($company) and !empty($company))
		...
		<?php $jobTitleError = (isset($errors) and $errors->has('current_job_title')) ? ' is-invalid' : ''; ?>
		<div class="form-group row required">
			<label class="{{ $classLeftCol }} control-label" for="current_job_title">{{t('Contact person job title')}} <sup>*</sup></label>
			<div class="{{ $classRightCol }}">
				<input name="current_job_title"
					placeholder="{{t('Contact person job title')}}"
					class="form-control input-md{{ $jobTitleError }}"
					type="text"
					value="{{ old('current_job_title', (isset($user->current_job_title) ? $user->current_job_title : '')) }}">
			</div>
		</div>
		...
		?>
		<?php $companyLocationError = (isset($errors) and $errors->has('company.company_location')) ? ' is-invalid' : ''; ?>
		<div class="form-group row required" id="companyLocation">
			<label class="{{ $classLeftCol }} control-label{{ $companyLocationError }}">{{t('Company Location')}} <sup>*</sup></label>
			<div class="{{ $classRightCol }}">
				<select class="form-control {{$companyLocationError}}" name="company[company_location]" id="company_location" placeholder="{{ t('Select') }}"
				value="{{ old('company.company_location') }}">
					<option value="" selected="">{{ t('Select') }}</option>
					@foreach ($companyLocationKeys as $item)
						<option value="{{$item}}" {{ (old("company.company_location", $company->company_location) == $item ? "selected":"") }} >{{t($item,[],'residenceCountry')}}</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		?>
		<?php $companySizeError = (isset($errors) and $errors->has('company.company_size')) ? ' is-invalid' : ''; ?>
		<div class="form-group row required" id="companySize">
			<label class="{{ $classLeftCol }} control-label{{ $companySizeError }}">{{t('Company size')}} <sup>*</sup></label>
			<div class="{{ $classRightCol }}">
				<select class="form-control {{$companySizeError}}" name="company[company_size]" id="company_size" placeholder="{{ t('Select') }}"
				value="{{ old('company.company_size') }}">
					<option value="" selected="">{{ t('Select') }}</option>
					@foreach ($companySizeKeys as $item)
						<option value="{{$item}}" {{ (old("company.company_size", $company->company_size) == $item ? "selected":"") }} >{{t($item,[],'companySize')}}</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		?>
		<?php $companyTypeError = (isset($errors) and $errors->has('company.company_type')) ? ' is-invalid' : ''; ?>
		<div class="form-group row required" id="companyType">
			<label class="{{ $classLeftCol }} control-label{{ $companyTypeError }}">{{t('Company type')}} <sup>*</sup></label>
			<div class="{{ $classRightCol }}">
				<select class="form-control {{$companyTypeError}}" name="company[company_type]" id="company_type" placeholder="{{ t('Select') }}"
				value="{{ old('company.company_type') }}">
					<option value="" selected="">{{ t('Select') }}</option>
					@foreach ($companyTypeKeys as $item)
						<option value="{{$item}}" {{ (old("company.company_type", $company->company_type) == $item ? "selected":"") }} >{{t($item,[],'companyType')}}</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		<div class="form-group row required">
			<label class="{{ $classLeftCol }} control-label{{ $companyCountryCodeError }}" for="company.country_code">{{t('phone country')}}</label>
			<div class="{{ $classRightCol }}">
		...
		<div class="form-group row">
			<label class="{{ $classLeftCol }} control-label" for="company.phone">{{t('Contact person phone')}}</label>
			<div class="input-group {{ $classRightCol }}">
				...
				<input name="company[phone]" type="text"
					   class="form-control{{ $companyPhoneError }}" placeholder="Ex:786666666"
		...
		<div class="form-group row">
			<label class="{{ $classLeftCol }} control-label" for="company.email">{{t('Contact person email')}}</label>
			<div class="input-group {{ $classRightCol }}">
				<div class="input-group-prepend">
