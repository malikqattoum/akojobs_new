			...
			<div class="navbar-identity">
				...
				<a href="{{ url(config('app.locale') . '/') }}" class="navbar-brand logo logo-title">
					@if((!auth()->check() && strpos(url()->current(), 'employers')) !== FALSE || auth()->check() && in_array(auth()->user()->user_type_id, [1]))
						<img src="{{ \Storage::url('app/logo/akojobs_emp.jpeg') }}" class="tooltipHere main-logo" />
					@elseif(auth()->check() || !auth()->check() || (auth()->check() && in_array(auth()->user()->user_type_id, [2])))
						<img src="{{ \Storage::url('app/logo/akojobs_js.jpeg') }}" class="tooltipHere main-logo" />
					@else
						<img src="{{ \Storage::url('app/logo/akojobs_js.jpeg') }}" class="tooltipHere main-logo" />
					@endif
			...
			<div class="navbar-collapse collapse">
				...
				<ul class="nav navbar-nav ml-auto navbar-right">
					@if (!auth()->check())
						...
						<li class="nav-item">
							<a href="{{ url(config('app.locale') . '/' . trans('routes.register')) }}" class="nav-link btn btn-block btn-post btn-add-listing">
							...
							<ul id="userMenuDropdown" class="dropdown-menu user-menu dropdown-menu-right shadow-sm">
								...
								@if (in_array(auth()->user()->user_type_id, [1]))
									<li class="dropdown-item">
										<a href="{{ url(config('app.locale') . '/account/companies') }}">
								...
								@if (in_array(auth()->user()->user_type_id, [2]))
									<li class="dropdown-item">
										<a href="{{ url(config('app.locale') . '/account/resumes') }}">
					...
					{{-- @if (!auth()->check())
						<li class="nav-item postadd">
							...
							@else
								<a class="btn btn-block btn-post btn-add-listing" href="{{ addPostURL(true) }}">
									<i class="fa fa-plus-circle"></i> {{ t('Create a Job ad') }}
								</a>
							@endif
						</li>
