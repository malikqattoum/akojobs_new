					...
					<div class="inner inner-box ads-details-wrapper pb-0">
						<h2 class="enable-long-words">
							<strong>
                                ...
                                <a href="{{ lurl($post->uri, $attr) }}" title="{{ $post->title }}" style="color:#ED3E00">
                                    {{ $post->title }}
                                </a>
                            </strong>
						...
						<div class="ads-details">
							<div class="row pb-4">
								<div class="ads-details-info jobs-details-info col-md-8 col-sm-12 col-xs-12 enable-long-words from-wysiwyg">
									<h5 class="list-title" style="color:#081C55"><strong>{{ t('Job Details') }}</strong></h5>
									...
									@if (!empty($post->company_description))
										<!-- Company Description -->
										<h5 class="list-title mt-5" style="color:#081C55"><strong>{{ t('Company Description') }}</strong></h5>
                                        <div>
									...
									@if (!empty($post->tags))
										...
										@if (!empty($tags))
											...
											<div class="tags">
												<h5 class="list-title" style="color:#081C55"><strong>{{ t('Tags') }}</strong></h5>
										...
										<ul>
											...
											<li>
												<p class="no-margin">
													...
													@if ($post->is_confidential)
														{{ t('confidential company') }}
													@else
							...
							<div class="content-footer text-left">
								@if (auth()->check())
									@if (auth()->user()->id == $post->user_id)
										<a class="btn btn-default" href="{{ editPostURL($post->id) }}">
											<i class="fa fa-pencil-square-o"></i> {{ t('Edit') }}
										</a>
										<a href="{{ url(config('app.locale') . '/posts/create') }}" target="_blank" class="btn btn-primary btn-flat pull-left">
								...
								<div class="card-body text-center">
									<div class="seller-info">
										<div class="company-logo-thumb mb20">
											@if ($post->is_confidential)
												<img alt="Logo {{ t('confidential company') }}" class="img-fluid" src="{{ resize('app/default/picture.jpg', 'big') }}">
											@else												
												@if (isset($post->company) and !empty($post->company))
										...
										@if (isset($post->company) and !empty($post->company))
											...
											<h3 class="no-margin">
												@if ($post->is_confidential)
													<span class="text-muted">{{ t('confidential company') }}</span>
												@else
													<a href="{{ lurl(trans('routes.v-search-company', $attr), $attr) }}">
														{{ $post->company->name }}
													</a>
									...
									<div class="user-ads-action">
										...
										@else
											@if ($post->email != '')
        										<a class="btn btn-default btn-block" {!! $applyLinkAttr !!} href="{{ $applyJobURL }}">
        											<i class="icon-mail-2"></i> {{ t('Apply Online') }}
        										</a>
        									@endif
											@if ($post->phone_hidden != 1 and !empty($post->phone))
