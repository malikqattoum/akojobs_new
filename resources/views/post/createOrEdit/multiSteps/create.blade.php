								...
								<form class="form-horizontal" id="postForm" method="POST" action="{{ url()->current() }}" enctype="multipart/form-data">
									...
									<fieldset>
										...
										<div class="form-group row required">
											...
											<div class="col-md-8">
												<select name="parent_id" id="parentId" class="form-control {{ $parentIdError }}" placeholder="{{ t('Select a category') }}">
														...
														@if (config('currency')['in_left'] == 0)
															<div class="input-group-append">
																...
																<input id="currency1"
																	name="currency"
																	type="radio"
																	value="IQD" {{ (old('salary_currency')=='IQD') ? 'checked="checked"' : '' }}
																	checked
																	>&nbsp;IQD
																<input id="currency2"
																	name="currency"
																	type="radio"
																	class="ml-2"
																	value="USD" {{ (old('salary_currency')=='USD') ? 'checked="checked"' : '' }}
																	>&nbsp;USD
																</span>
																{{-- <span class="input-group-text">{!! config('currency')['symbol'] !!}</span> --}}
															</div>
										...
										*/
										...
										<div class="form-group row">
											...
											<div class="col-md-8">
												<input id="tags" name="is_confidential" placeholder="{{ t('hide company details') }}" class="input-md mt-2" type="checkbox" value="1" {{(old('is_confidential'))?'checked':''}}>
											</div>
										</div>
