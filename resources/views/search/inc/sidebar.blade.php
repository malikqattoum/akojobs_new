				...
				<ul class="browse-list list-unstyled long-list">
					@if (isset($cities) and $cities->count() > 0)
						@foreach ($cities as $city)
							...
							?>
							@if(!empty(!empty($city->posts->count())))
								<li>
									...
									@else
										<?php 
											...
											$requestParams = request()->except(['page'] + array_keys($locationParams));
											$requestParams['location'] = $city->name;
										?>
										<a href="{!! qsurl($fullUrlLocation, array_merge($requestParams, $locationParams), null, false) !!}" title="{{ $city->name }}">
											@if($locale == 'en')
												{{ $city->name }}
											@else
												{{ $city->ar_name }}
