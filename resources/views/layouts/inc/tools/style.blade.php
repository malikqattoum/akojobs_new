...
$titleColor = config('settings.style.title_color');
...
$footerTitleColor = config('settings.style.footer_title_color');
if((!auth()->check() && strpos(url()->current(), 'employers')) !== FALSE || auth()->check() && in_array(auth()->user()->user_type_id, [1]))
...
if (!empty($footerTitleColor)) {
	$cssBuffer .= '.footer-title { color: ' . $footerTitleColor . '; }';
}
