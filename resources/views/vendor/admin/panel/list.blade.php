...
@section('content')
<section class="content" style="padding-right:0">
						...
						<table id="crudTable" class="table table-bordered table-striped display dt-responsive nowrap" width="100%">
							...
							<tr>
								...
								@foreach ($xPanel->columns as $column)
                                    ...
                                    @elseif($column['label'] == "Note")
                                        <th {{ isset($column['orderable']) ? 'data-orderable=' .var_export($column['orderable'], true) : '' }} class="sorting not-exported">
                                            {!! $column['label'] !!}
                                        </th>
                                    @else
										<th {{ isset($column['orderable']) ? 'data-orderable=' .var_export($column['orderable'], true) : '' }}>
								...
								@if ( $xPanel->buttons->where('stack', 'line')->count() )
									<th data-orderable="false" class="not-exported">{{ trans('admin::messages.actions') }}</th>
								@endif
							</tr>
							</thead>
							...
							<tr>
								...
								@if ( $xPanel->buttons->where('stack', 'line')->count() )
									<th class="not-exported">{{ trans('admin::messages.actions') }}</th>
								@endif
							</tr>
							</tfoot>
						</table>
        ...
        jQuery(document).ready(function($) {
			...
			$('#bulkDeleteBtn').click(function(e) {
				...
				return false;
            			...
            			/* Bulk Items Deletion */
			$('#bulkVerifyBtn').click(function(e) {
				e.preventDefault();
		
				var atLeastOneItemIsSelected = $('input[name="entryId[]"]:checked').length > 0;
		
				if (atLeastOneItemIsSelected) {
					if (confirm("{{ trans('admin::messages.Are you sure you want to send verification email to the selected items?') }}") == true) {
						if (isDemo()) {
							return false;
						}
                        var entryValues = [];
                        ...
                        var verifyUrl = "{{ url('/admin/verify/user/resend/email') }}"
                        $.ajax({
                            url: verifyUrl,
                            type: 'POST',
                            data:{entryIds:entryValues},
                            success: function(result) {
                                // Show an alert with the result
                                new PNotify({
                                    title: "Emails sent",
                                    text: "Verification emails have been sent to the selected emails",
                                    type: "success"
                                });
                            },
                            error: function(result) {
            ...
            $('#bulkCompleteProfileBtn').click(function(e) {
				e.preventDefault();
		
				var atLeastOneItemIsSelected = $('input[name="entryId[]"]:checked').length > 0;
		
				if (atLeastOneItemIsSelected) {
					if (confirm("{{ trans('admin::messages.Are you sure you want to send complete profile email to the selected items?') }}") == true) {
						if (isDemo()) {
							return false;
						}
                        var entryValues = [];
                        ...
                        var verifyUrl = "{{ url('/admin/complete-profile/user/resend/email') }}"
                        $.ajax({
                            url: verifyUrl,
                            type: 'POST',
                            data:{entryIds:entryValues},
                            success: function(result) {
                                ...
                                new PNotify({
                                    title: "Emails sent",
                                    text: "Complete profile emails have been sent to the selected emails",
                                    type: "success"
                                });
                            },
                            error: function(result) {
             ...
             /* send upload resume email */
             $('#bulkUploadResumeBtn').click(function(e) {
				e.preventDefault();
		
				var atLeastOneItemIsSelected = $('input[name="entryId[]"]:checked').length > 0;
		
				if (atLeastOneItemIsSelected) {
					if (confirm("{{ trans('admin::messages.Are you sure you want to send upload resume email to the selected items?') }}") == true) {
						if (isDemo()) {
							return false;
						}
                        var entryValues = [];
                        ...
                        var verifyUrl = "{{ url('/admin/upload-resume/user/resend/email') }}"
                        $.ajax({
                            url: verifyUrl,
                            type: 'POST',
                            data:{entryIds:entryValues},
                            success: function(result) {
                                ...
                                new PNotify({
                                    title: "Emails sent",
                                    text: "Upload resume emails have been sent to the selected emails",
                                    type: "success"
                                });
                            },
                            error: function(result) {
            ...
            /* send custom emails */
            $('#bulkSendCustomEmailBtn').click(function(e) {
				e.preventDefault();
                tinyMCE.triggerSave(true, true);
                var formData = $('#sendCustomEmailForm').serializeArray();
				var atLeastOneItemIsSelected = $('input[name="entryId[]"]:checked').length > 0;

				if (atLeastOneItemIsSelected) {
					if (confirm("Are you sure you want to send this custom email to the selected users?") == true) {
						if (isDemo()) {
							return false;
						}
                        var entryValues = [];
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                ...
                                new PNotify({
                                    title: "Emails sent",
                                    text: "Custom email been sent to the selected users",
                                    type: "success"
                                });
                            },
                            error: function(result) {
             ...
             /* send invoice emails */
             $('#bulkSendInvoiceEmailBtn').click(function(e) {
				e.preventDefault();
                tinyMCE.triggerSave(true, true);
                var formData = $('#sendInvoiceEmailForm').serializeArray();
				var atLeastOneItemIsSelected = $('input[name="entryId[]"]:checked').length > 0;

				if (atLeastOneItemIsSelected) {
					if (confirm("Are you sure you want to send this invoice email to the selected users?") == true) {
						if (isDemo()) {
							return false;
						}
                        var entryValues = [];
                        ...
                        formData.push({name:"entryIds", value:entryValues});
                        console.log(formData);
                        var verifyUrl = "{{ url('/admin/invoice-email/user/resend/email') }}"
                        $.ajax({
                            url: verifyUrl,
                            type: 'POST',
                            data:formData,
                            success: function(result) {
                                ...
                                new PNotify({
                                    title: "Emails sent",
                                    text: "Invoice email been sent to the selected users",
                                    type: "success"
                                });
                            },
                            error: function(result) {
