...
trait EditTrait
{
	...
	public function getUpdateForm($postIdOrToken)
	{
		...
		$data['cities'] = City::where([['active', '=', 1], ['country_code', '=', 'IQ']])->get();
	...
	public function postUpdateForm($postIdOrToken, PostRequest $request)
	{
		...
		if ($request->filled('company_id') && !empty($request->input('company_id'))) {
			...
			$company = Company::where('id', $request->input('company_id'))->first();		} else {
		...
		$minSalary = str_replace('.','',$request->input('salary_min'));
		$maxSalary = str_replace('.','',$request->input('salary_max'));
		
		$minSalary = floatval($minSalary);
		$maxSalary = floatval($maxSalary);
