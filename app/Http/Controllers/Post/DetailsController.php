...
class DetailsController extends FrontController
{
	...
	public function index($postId)
	{
		...
		if (isset($post->logo) && !empty($post->logo)) {
			...
			$this->og->image(resize($post->logo, 'big'), [
				'width'  => 600,
				'height' => 600,
			]);
		}
	...
	public function sendMessage($postId, SendMessageRequest $request)
	{
		...
		$user = DB::table('users')->where('email',$request->input('from_email'))->first();
		...
		if ($request->filled('resume_id') && !empty($request->input('resume_id'))) {
			...
			} elseif(!auth()->check() && !empty($user) && is_object($user)) {
				if (!isset($resumeInfo['user_id']) || empty($resumeInfo['user_id'])) {
					$resumeInfo += ['user_id' => $user->id];
				}
...
// 		$resume->save();
		...
		if (!auth()->check()) {
			...
			{
				$user = new User();
				$user->name = $request->input('from_name');
				$user->email = $request->input('from_email');
				$user->user_type_id = 2;
				$user->password = Hash::make("AkojobsNewCandidatePassw0rd!");
				$user->residence_country = $this->getCountryCodeFromIP();
				$user->phone = $request->input('from_phone');
				$user->country_code   = $this->getCountryCodeFromIP();
				$user->user_source   = 3;
			...
			$resume->filename = $message->filename;
			$resume->save();
	
			$userSource = new UserSource();
			$userSource->added_by_user_id = 0;
			$userSource->email = $request->input('from_email');
			$userSource->source = '3';
			$userSource->save();
		}
		...
		try {
			...
			\Debugbar::info($post);
			$msg = t("Your message has sent successfully to :contact_name.", ['contact_name' => $post->company_name]);
			flash($msg)->success();
		} catch (\Exception $e) {
	...
	public function checkIfRegistered(Request $request)
	{
	    ...
	    if(!empty($user))
	    {
	        return ["message"=>'You are a registered user, would you like to <a href="'.lurl(trans('routes.login')).'">login</a> or continue without login'];
	    }
