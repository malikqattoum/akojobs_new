...
trait PreSearchTrait
{
	...
	public function getCity($cityId = null, $location = null)
	{
		...
		//die(var_dump(City::currentCountry()->where('name', 'LIKE', $cityName)->orWhere('ar_name', 'LIKE', $cityName)->first()));
		
		// Get City by Name
		if (empty($this->city) && !empty($location)) {
			$this->city = City::currentCountry()->where('name', 'LIKE', $cityName)->orWhere('ar_name', 'LIKE', $cityName)->first();
			if (empty($this->city)) {
				$this->city = City::currentCountry()->where('name', 'LIKE', $cityName . '%')->orWhere('ar_name', 'LIKE', $cityName . '%')->first();
				if (empty($this->city)) {
					$this->city = City::currentCountry()->where('name', 'LIKE', '%' . $cityName)->orWhere('ar_name', 'LIKE', '%' . $cityName)->first();
