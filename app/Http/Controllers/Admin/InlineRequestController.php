...
class InlineRequestController extends Controller
{
	...
	public function make($table, $field, Request $request)
	{
		...
		} elseif($table == 'packages_requests' && $field == 'active') {
			if($item->{$field} != 1)
			{
				$requestedPackage = PostPackage::findOrFail($item->package_id);
				$item->approve_date = date('Y-m-d', time());
				$period = $requestedPackage->period;
				$period = (string) $period;
				$item->end_date = date('Y-m-d',strtotime("+".$period." day", time()));
			} 
			else
			{
