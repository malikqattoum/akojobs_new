...
trait VerifiedTrait
{
                    ...
                    if (!empty($entity) && isset($entity->provider)) {
                        if (!empty($entity->provider)) {
							...
							if ($entity->provider == 'linkedin') {
								...
								$out .= ' &nbsp;<i class="admin-single-icon fa fa-linkedin-square" style="color: #0B2271;" ' . $toolTip . '></i>';
							}
    ...
    public function getJobApplicantsCount()
    {
        $out = '';
        $out .= '<a href="'.lurl('account/conversations/'.$this->id.'/applicants').'">';
        $out .= $this->messages->where('parent_id', 0)->count();
        $out .= '</a>' ;
        return $out;
    }
}